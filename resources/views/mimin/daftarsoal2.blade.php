<x-admin-layout>
    ...
    <div class="box1" style="margin-top: 20px;">
        <div style="padding:10px;">
            <a href="{{route('daftar-soal2.create')}}"><button class="bg-green-700 hover:bg-green-800 text-white font-bold py-2 px-3" style="border-radius:6px;">Tambah Data</button></a>
        ...
        <form action="{{route('daftar-soal2.index')}}" method="GET">
        ...
        <table class="w-full p-5 mt-2">
            ...
            <tbody>
                ...
                @foreach ($data as $datasoal2 => $soal2)
                <tr class="text-center text-black">
                    <td>{{$no}}</td>
                    <td>{{$soal2->no_soal}}</td>
                    <td>{{$soal2->kategori}}</td>
                    <td>{{Str::limit($soal2->soal, 20)}}</td>
                    <td>
                        <form action="{{route('daftar-soal2.destroy', $soal2->id)}}" method="POST">
                            <a href="{{route('daftar-soal2.edit', $soal2->id)}}" class="bg-yellow-400 py-1 px-4 hover:bg-yellow-600 rounded text-white font-bold" style="padding-top:7px; padding-bottom:7px;">Edit</a>
